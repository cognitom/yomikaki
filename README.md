...
ページ内 fetch の挙動も本番と食い違う。`http://` で開いて確認すること。
...
`shim/` にあるのは `node` だけで `npm` は無いため、`npm run serve` はこの方法では動かない。
node を入れていない場合にこの shim で代替できるのは、上のテストのように `node` を直接呼ぶ
コマンドだけ。使い捨ての Docker コンテナで実行され、常駐するものは残らない。
